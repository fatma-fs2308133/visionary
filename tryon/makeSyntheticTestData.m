function [frame, keypoints, garmentImg, garmentAlpha] = makeSyntheticTestData(brightness)
...
skin  = [0.86 0.70 0.58];
shirt = [0.16 0.20 0.30];          % the (real) dark shirt the user wears
...
frame = frame * brightness + 0.015 * randn(H, W, 3);
frame = im2uint8(min(max(frame, 0), 1));
